/**
 * Heavy implementation of the Help overlay (Phase 65 lazy split).
 *
 * The thin stub at `./help-overlay.ts` mounts only the floating "?"
 * button + key handlers at boot; on first interaction it dynamic-
 * imports this module, calls `buildHelpOverlay(viewMode, button)`
 * and hands off to the returned `{open, close, toggle}` handle.
 *
 * Why split: the section text + modal markup is ~8.5 KB brotli —
 * the largest single module in the shared `los-compose` chunk.
 * Most users open the help overlay rarely (or never); deferring
 * its load to first-use takes that weight out of the initial
 * critical path.
 */
...
const GM_SECTIONS: HelpSection[] = [
  ...
  },
  {
    ...
    entries: [
      ...
      { name: 'Connection status', desc: 'Once a peer is connected, a small chip appears in the top-left of your view ("Connected" / "Connecting…" / "Disconnected…"). Clicking it re-opens the Remote Play modal where you can Disconnect gracefully or start a new invitation to reconnect.' },
      { name: 'Reconnecting', desc: 'If the connection drops (network blip, the other end reloads, NAT change), the status chip turns red and the Spectator side gets a banner prompting a reconnect. Open Remote Play, click Disconnect to clear the old state, then walk through the Host/Join flow again. Your local session state is preserved — the GM re-broadcasts a full-state to the freshly-connected peer automatically.' },
      { name: 'Limitations', desc: 'Phase 62/64 is a manual-signaling MVP: no room codes, no server-backed auto-reconnect, no trickle ICE (so connection setup can take 2–5 seconds). If both peers are behind symmetric NATs the connection may fail — exporting + importing a session file is a reliable fallback.' },
    ],
  },
  {
...
/**
 * Build the help overlay around an already-mounted "?" button.
 * Called by the lazy stub in `./help-overlay.ts` on first user
 * interaction. Returns the same `{open, close, toggle}` handle
 * the stub exposes synchronously (proxied through a shared box).
 */
export function buildHelpOverlay(
  ...
  // The button click is handled by the stub in `./help-overlay.ts`
  // so the first click can trigger the lazy import; once loaded,
  // the stub forwards every subsequent click to our `toggle()`.
  // (`button` parameter unused at this stage but reserved for
  // future positional / focus tweaks.)
  void button;
